...
namespace NuGenBioChem
{
    ...
    public partial class Window : RibbonWindow
    {
        ...
        {
            /*REMOVE IT : BEGIN*/
            Render render = new Render(substance, style);
            visualizer1.Render = render;
            /*REMOVE IT : END*/
        ...
        {
            if (!Render.IsHardwareCompatible)
            {
                // FIXME: closes immediately after show! Try it with Fluent 2.0
                MessageBox.Show("NuGenBioChem is required hardware with Pixel Shader 2.0", "NuGenBioChem", MessageBoxButton.OK, MessageBoxImage.Stop);
